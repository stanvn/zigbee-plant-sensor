/* auto-generated by gen_syscalls.py, don't edit */

#include <syscalls/socket_select.h>

extern int z_vrfy_zsock_select(int nfds, zsock_fd_set * readfds, zsock_fd_set * writefds, zsock_fd_set * exceptfds, struct zsock_timeval * timeout);
uintptr_t z_mrsh_zsock_select(uintptr_t arg0, uintptr_t arg1, uintptr_t arg2,
		uintptr_t arg3, uintptr_t arg4, uintptr_t arg5, void *ssf)
{
	_current->syscall_frame = ssf;
	(void) arg5;	/* unused */
	union { uintptr_t x; int val; } parm0;
	parm0.x = arg0;
	union { uintptr_t x; zsock_fd_set * val; } parm1;
	parm1.x = arg1;
	union { uintptr_t x; zsock_fd_set * val; } parm2;
	parm2.x = arg2;
	union { uintptr_t x; zsock_fd_set * val; } parm3;
	parm3.x = arg3;
	union { uintptr_t x; struct zsock_timeval * val; } parm4;
	parm4.x = arg4;
	int ret = z_vrfy_zsock_select(parm0.val, parm1.val, parm2.val, parm3.val, parm4.val);
	_current->syscall_frame = NULL;
	return (uintptr_t) ret;
}

